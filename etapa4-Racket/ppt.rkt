...
(define ppt-stream-in-tree-order
  (let iter ((stream (stream-cons '(3 4 5) empty-stream)))
    (stream-cons (stream-first stream) (iter (stream-append (stream-rest stream) (ppt-next-3-to-stream (stream-first stream)))))))

(define (ppt-next-3-to-stream startpos)
  (stream-cons (multiply T1 startpos) (stream-cons (multiply T2 startpos) (stream-cons (multiply T3 startpos) '()))))
...
(define (interclasare flux1 flux2)
  (if (<= (cdr (stream-first flux1)) (cdr (stream-first flux2)))
      (stream-cons (stream-first flux1) (interclasare (stream-rest flux1) flux2))
      (stream-cons (stream-first flux2) (interclasare flux1 (stream-rest flux2)))))
      
(define (pairs G H)
  (stream-cons (cons (stream-first G) (stream-first H))
               (interclasare (stream-map (lambda (elem) (cons (stream-first G) elem)) (stream-rest H))
...
; generare flux de numere impare G
; conditie g < h
; g,h prime intre ele - cel mai mare divizor comun al lor sa fie 1
...
(define (prime-nr flux)
  (stream-filter (lambda (pair) (= (gcd (car pair) (cdr pair)) 1)) flux))

(define gh-pairs-stream
  (prime-nr (pairs (stream-filter odd? naturalsG) (stream-filter odd? naturalsH))))
...
(define ppt-stream-in-pair-order
  (stream-map (lambda (pair) (list
                              (* (car pair) (cdr pair))
                              (/ (- (* (cdr pair) (cdr pair)) (* (car pair) (car pair))) 2)
                              (/ (+ (* (cdr pair) (cdr pair)) (* (car pair) (car pair))) 2)))
  (prime-nr (pairs (stream-filter odd? naturalsG) (stream-filter odd? naturalsH)))))