...
(define (dot-product X Y)
  (foldl (lambda (x y rez) (+ rez (* x y)))  X Y 0))
...
;     |-2 2 3| |5|   |17|
(define (multiply M V)
    (map (lambda (row) (apply + (map * row V))) M)) ; aplic functia pt toata matricea M

; primesc elementul de pe linia i, il inmultesc cu elementul i din vector, adun rezultatul cu urmatorul, pana ajung la ultimul element din linia respectiva
; apoi , trec la urmatoarea linie din matrice
...
(define (apply-functional-transformations Fs tuple)
   (car (foldl (λ (func list) (map func list)) (list tuple) Fs))) ; aplic foldl pe toata lista de functii Fs si pe acumulatorul initial care e lista de tuple
...
; start -> FS'(W1, W2, W3) -> n ->    al n-lea termen
; (3 1 3 1) =?=> W3 W1 W3 W1 ==> apply-functional-transformations (W3 W1 W3 W1) start
...
(define apply-q ; aplica q pe tuplul dat
  (lambda (Qn)
    (lambda (tuplu)
     (apply Qn tuplu))))

(define TF1 ; aplica functia corespunzatoare t-ului pe tuplu-adica inmultirea dintre T si tuplu
  (lambda (tuplu) (multiply T1 tuplu)))
(define TF2
  (lambda (tuplu) (multiply T2 tuplu)))
(define TF3
  (lambda (tuplu) (multiply T3 tuplu)))
...
(define generate-Tn-apply ;functie ajutatoare, primeste transformare Tn si inmulteste Tn cu tuplul initial
  (lambda(Tn) 
    (lambda(tuplu) (multiply Tn tuplu))))

(define get-nth-ppt-from-matrix-transformations (get-nth-tuple '(3 4 5) (generate-Tn-apply T1) (generate-Tn-apply T2) (generate-Tn-apply T3))) ; minimul de efort
...
; y = d^2 - c^2 - b^2
; z = d^2 - c^2 - b^2 + a^2
; => obtinut tripletul pentru un anumit n = (c^2 - b^2 d^2 - c^2 - b^2 d^2 - c^2 - b^2 + a^2)

; toate cele 3 functii ajutatoare primesc ca parametru lista de cvartete obtinuta din get-nth-quadruple n , si obtin termenii pt get-nth-ppt ulterior
(define first-term ; x
  (lambda (list) (- (* (caddr list) (caddr list)) (* (cadr list) (cadr list)) ) )) 

(define second-term ; y
  (lambda (list) (- (* (cadddr list) (cadddr list)) (* (caddr list) (caddr list)) (* (cadr list) (cadr list)) )))

(define last-term ; z
  (lambda (list) (+ (- (* (cadddr list) (cadddr list)) (* (caddr list) (caddr list)) (* (cadr list) (cadr list)) ) (* (car list) (car list)))))

(define get-nth-ppt-from-GH-quadruples
  (lambda (n)
    (list (first-term (get-nth-quadruple n)) (second-term (get-nth-quadruple n)) (last-term (get-nth-quadruple n)))))
